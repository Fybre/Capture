...
# Build outside synced workspace folders. macOS File Provider can immediately add Finder/provenance
# xattrs to a bundle created under Documents between xattr cleanup and codesign, causing codesign to
# reject an otherwise clean local build with "resource fork, Finder information, or similar detritus".
BUILD_DIR="$(mktemp -d "${TMPDIR:-/tmp}/CaptureScanHelperMac.XXXXXX")"
