...
- **Import & batching** — watch folders, scanning, or manual import through unified Capture Profiles
  with batch boundaries, document classification, and document separation.
...
This version uses an isolated `CaptureV2` application-data directory, so it can run beside the previous Capture version without sharing its database, settings, profiles, work files, or models.

The workflow designer is available under **More → Capture profiles…**. Its **Test Capture** section runs the same analyse → plan → materialise workflow used by imports, scanning, and watch folders.
