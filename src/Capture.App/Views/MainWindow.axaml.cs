...
public partial class MainWindow : Window
{
    ...
    private void OnGridPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        // The hover-only copy action operates on the existing multi-selection. Do not let the grid's
        // drag/selection tunnel collapse selections in other profile groups before its Click runs.
        if ((e.Source as Visual)?.FindAncestorOfType<Button>(includeSelf: true) is { } pressedButton
            && pressedButton.Classes.Contains("table-index-copy"))
            return;
    ...
    private DataGridTemplateColumn BuildIndexColumn(string fieldName, FontFamily monoFont, bool isBatchField)
    {
        return new DataGridTemplateColumn
        {
            ...
            CellTemplate = new FuncDataTemplate<DocumentRow>((_, _) =>
            {
                ...
                copy.Classes.Add("table-index-copy");
                ToolTip.SetTip(copy, "Copy this value into the same field on every other selected document");
                copy.Click += OnTableIndexCopyClick;
                ...
                {
                    var canCopy = DataContext is MainViewModel viewModel
                        && viewModel.HasMultipleSelectedDocuments
                        && cell.DataContext is DocumentRow source
    ...
    private async void OnTableIndexCopyClick(object? sender, RoutedEventArgs e)
    {
        if (sender is not Button
            {
                DataContext: DocumentRow source,
                Tag: IndexCellBinding { IsBatchField: false } request
            }
            || DataContext is not MainViewModel viewModel)
            return;

        e.Handled = true;
        await viewModel.ApplyTableFieldToSelectionAsync(source, request.FieldName);
    }
