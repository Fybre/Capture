...
public partial class MainViewModel
{
    ...
    public bool HasSelectedAdjustableRedaction => SelectedRedactionCandidate?.CanAdjustBounds == true;
    ...
    [NotifyPropertyChangedFor(nameof(HasSelectedAdjustableRedaction))]
        ...
        try
        {
            ...
            foreach (var row in rows)
            {
                ...
                try
                {
                    ...
                    await _redactionDetection.DetectAsync(row.Document, pages, row.DocumentIndexes, settings).ConfigureAwait(true);
                    row.NotifyIndexes();
                }
                catch (Exception ex)
                {
    ...
    [RelayCommand]
    private void ChangeRedaction(NormalizedRect rect)
    {
        if (SelectedRedactionCandidate is not { CanAdjustBounds: true } row)
            return;
    ...
    [RelayCommand]
    private void OpenRedactedFile(DocumentRow? row)
    {
        var path = (row ?? SelectedDocument)?.Document.RedactedPath;
