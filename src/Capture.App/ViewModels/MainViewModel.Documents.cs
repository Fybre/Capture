...
public partial class MainViewModel
{
    ...
    private async Task MarkReadyAsync()
    {
        ...
        try
        {
            ...
            // meant to make.
            if (FindDocumentType(document.ProfileId) is { } profile)
            {
                var indexes = await _indexes.GetAsync(document.Id).ConfigureAwait(true);
                await RunPostIndexStepsAsync(document, indexes, profile).ConfigureAwait(true);
    ...
    {
        if (_watchSettings.DuplicateImportBehavior != DuplicateImportBehavior.FlagForReview)
        {
            foreach (var row in Documents)
            {
        ...
        var groups = Documents
            ...
            .Where(group => group.Select(row => DuplicateSourceIdentity.For(row.Document)).Distinct().Count() > 1)
        ...
        foreach (var row in Documents)
        {
            ...
            {
                var sourceOccurrence = DuplicateSourceIdentity.For(row.Document);
                var others = duplicates
                    .Where(item => DuplicateSourceIdentity.For(item.Document) != sourceOccurrence)
    ...
    private void RefreshDocumentGroups()
    {
        ...
        {
            var profile = FindDocumentType(byProfile.Key);
            // GroupBy preserves the order of the shared Documents collection, so the Table view now
