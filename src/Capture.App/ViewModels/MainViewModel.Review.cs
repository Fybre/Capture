...
public partial class MainViewModel
{
    ...
    {
        // Batch fields deliberately do not come through this path. They already propagate to every
        // document in their batch when edited, while a Table selection can span unrelated batches.
        // Read-only is also enforced here, not just in button visibility, so programmatic invocation
        // can never turn this convenience action into a way around the profile's edit policy.
        ...
        {
            var match = target.DocumentIndexes.FirstOrDefault(item => item.FieldId == sourceValue.FieldId);
            if (match is null)
                continue;
            if (match.IsReadOnly)
            {
        ...
        StatusText = applied > 0
            ? $"Applied \"{sourceValue.FieldName}\" to {applied} other document{(applied == 1 ? "" : "s")}"
                + (skippedReadOnly > 0 ? $"; {skippedReadOnly} read-only skipped" : string.Empty)
            : skippedReadOnly > 0
