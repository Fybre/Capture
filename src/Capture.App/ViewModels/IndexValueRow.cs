...
public sealed partial class IndexValueRow : ObservableObject
{
    ...
    /// carry one (batch-level fields live on the capture profile's batch definition).</summary>
    public bool IsBatch { get; }

    public string ScopeLabel => IsBatch ? "Batch" : "Document";

    /// <summary>Sensitive batch fields are shared metadata, not page-redaction targets. Keep their
    /// plaintext editable but masked by default in review, with an explicit per-row reveal.</summary>
    public bool IsSensitiveBatch => IsBatch && Value.Sensitive;

    public char PasswordChar => IsSensitiveBatch ? '●' : '\0';

    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(SensitiveValueToggleLabel))]
    ...
    public string SensitiveValueToggleLabel => RevealSensitiveValue ? "Hide" : "Show";

    public bool IsSensitiveBatchMasked => IsSensitiveBatch && !RevealSensitiveValue;
    ...
    public string ButtonStatus => IsSensitiveBatchMasked && !string.IsNullOrEmpty(Value.Value) ? "••••••" : Value.Value;
