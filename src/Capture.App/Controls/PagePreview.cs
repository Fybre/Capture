...
public sealed class PagePreview : Control
{
    public PagePreview()
    {
        // Custom drawing is not clipped automatically. Keeping this on the control itself prevents
        // a zoomed page from painting over adjacent designer panels regardless of its host layout.
        ClipToBounds = true;
    }
    ...
    public static readonly StyledProperty<bool> AllowHighlightEditProperty =
        AvaloniaProperty.Register<PagePreview, bool>(nameof(AllowHighlightEdit));
    ...
    /// <summary>Allows an already-selected highlight with CanEdit=true to be moved or resized without
    /// also enabling blank-area drawing. Review uses this for detected redactions; the profile designer
    /// continues to couple editing and drawing through AllowDraw.</summary>
    public bool AllowHighlightEdit
    {
        get => GetValue(AllowHighlightEditProperty);
        set => SetValue(AllowHighlightEditProperty, value);
    }
    ...
    private void ObserveHighlights()
    {
        if (_observedHighlights is not null)
            _observedHighlights.CollectionChanged -= OnHighlightsCollectionChanged;
        _observedHighlights = Highlights as INotifyCollectionChanged;
        if (_observedHighlights is not null)
            _observedHighlights.CollectionChanged += OnHighlightsCollectionChanged;
        InvalidateVisual();
    }
    ...
    protected override void OnPointerPressed(PointerPressedEventArgs e)
    {
        ...
        {
            var selected = SelectedHighlight();
            if ((AllowDraw || AllowHighlightEdit) && selected is { CanEdit: true })
            {
