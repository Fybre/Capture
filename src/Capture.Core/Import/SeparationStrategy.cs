...
/// <summary>How a capture profile rule set combines rules into a match.</summary>
public enum SeparationMatchMode
{
    ...
    /// <summary>At least the rule set's configured minimum number of rules must match.</summary>
    AtLeast = 2,

    /// <summary>Disable this rule set. Stored rules are retained but are not evaluated.</summary>
    None = 3
}
...
/// <summary>
/// One condition inside a capture profile rule set. Like <c>IndexField</c>
...
public sealed class SeparationStrategy
{
    ...
    /// <summary>Only meaningful for <see cref="SeparationStrategyType.Barcode"/> and
    /// <see cref="SeparationStrategyType.OcrZone"/> — drawn against the profile's shared
    /// containing definition's shared sample, not a sample of this rule's own.</summary>
