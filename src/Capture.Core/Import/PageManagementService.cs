...
/// counterpart to capture-time page splitting, whose conventions this mirrors: for a PDF-sourced
...
public sealed class PageManagementService : IPageManagementService
{
        ...
        IReadOnlyList<Guid> documentIds, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // The merged bytes no longer represent either original source occurrence. Keeping the
            // first document's provenance would make a later import of that original look duplicated
            // even though the stored document now contains additional pages.
            target.ContentHash = null;
            target.SourceImportId = null;
    ...
    /// capture materialization: new id, PDF subset (or whole-file copy for image
        ...
        CancellationToken cancellationToken)
    {
        ...
        var document = new CaptureDocument
        {
            ...
            // A manual split is still part of the same source occurrence. Preserve both values so
            // either surviving half continues to participate in duplicate detection without the two
            // halves incorrectly flagging one another.
            ContentHash = original.ContentHash,
            SourceImportId = original.SourceImportId
        };
        await _store.SaveAsync(document, newPages, cancellationToken).ConfigureAwait(false);
        ...
        await RemapIndexValuesAsync(
            original.Id, id, oldToNew, inheritUnmappedValues: true, cancellationToken).ConfigureAwait(false);
        await RemapRedactionCandidatesAsync(original.Id, document, oldToNew, cancellationToken).ConfigureAwait(false);
    ...
    /// <summary>Remaps <c>indexes.json</c> to the new page numbering. A normal page deletion drops a
    /// zonal value when its physical source page is removed. A split is different: the new half inherits
    /// every populated document value as metadata, because values commonly come from a separator/header
    /// page that is deliberately absent from the retained document. An inherited value whose page did
    /// not move has its now-invalid bounds removed and is reattached to page 1.</summary>
    private async Task RemapIndexValuesAsync(
        Guid documentId, IReadOnlyDictionary<int, int> oldToNew, bool inheritUnmappedValues, CancellationToken cancellationToken)
        => await RemapIndexValuesAsync(documentId, documentId, oldToNew, inheritUnmappedValues, cancellationToken)
            .ConfigureAwait(false);
    ...
    private async Task RemapIndexValuesAsync(
        ...
        bool inheritUnmappedValues,
        CancellationToken cancellationToken)
    {
        var values = await _indexValues.GetAsync(sourceDocumentId, cancellationToken).ConfigureAwait(false);
        ...
        foreach (var value in values)
        {
            ...
            else if (inheritUnmappedValues && !string.IsNullOrWhiteSpace(value.Value))
            {
                value.PageNumber = 1;
                value.Bounds = null;
                remapped.Add(value);
            }
            else if (value.Bounds is not null)
            {
                continue; // page-local value whose source page was deleted
            }
            else
            {
                value.PageNumber = 1;
        ...
        await _indexValues.SaveAsync(targetDocumentId, remapped, cancellationToken).ConfigureAwait(false);
    }
    ...
    private async Task RemapRedactionCandidatesAsync(
        ...
        => await RemapRedactionCandidatesAsync(document.Id, document, oldToNew, cancellationToken)
            .ConfigureAwait(false);
    ...
    private async Task RemapRedactionCandidatesAsync(
        ...
        CancellationToken cancellationToken)
    {
        var candidates = await _redactionCandidates.GetAsync(sourceDocumentId, cancellationToken).ConfigureAwait(false);
