...
public interface IDocumentStore
{
    ...
    /// already-imported source currently has any duplicates, without persisting a separate flag that
    /// could go stale. Results can include sibling documents with the same
    /// <see cref="CaptureDocument.SourceImportId"/>; callers displaying duplicate relationships must
    ...
    /// user's perspective (e.g. capture materialization or page management cleaning up after a failed
...
public enum DuplicateImportBehavior
{
    ...
    /// <summary>Import it anyway, but documents belonging to distinct source imports with the same hash
    /// show a visible "matches an already-imported document" indicator. Documents split from one source
    /// occurrence are siblings and do not flag one another.</summary>
    FlagForReview = 2
}
