...
public sealed class RedactionApplier
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var previousRedactedPath = document.RedactedPath;
        try
        {
        ...
        {
            // Never leave a failed re-application pointing at an older or partially-written artifact.
            // Exporters also require Applied status, but clearing the path keeps the persisted state
            // internally consistent for every caller.
            if (!string.IsNullOrWhiteSpace(previousRedactedPath) && File.Exists(previousRedactedPath))
            {
                try { File.Delete(previousRedactedPath); }
                catch { /* The failed state and cleared path still prevent this artifact being exported. */ }
            }
            ...
            {
                if (!string.IsNullOrWhiteSpace(previousRedactedPath) && File.Exists(previousRedactedPath))
                    File.Delete(previousRedactedPath);
            }
            catch
            {
                // The failed state and cleared path are authoritative. Cleanup is best effort only.
            }
        }
