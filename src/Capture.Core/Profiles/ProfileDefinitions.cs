...
public enum FieldKind
{
    ...
    /// <summary>Mirrors whatever value the batch boundary rule (barcode/regex) captured for this
...
public sealed class IndexField
{
    ...
    /// <summary>An optional fallback template evaluated when normal extraction leaves this field blank.
    /// Text fields always use it as their initial value. Plain text passes through unchanged, and
    /// document/batch/page counters, date/time, profile name, and other field-name tokens are resolved.
    /// Null/empty means no fallback.</summary>
    ...
    /// candidate on documents whose document type has redaction enabled.</summary>
    ...
    /// <summary>Available on any field kind except <see cref="FieldKind.Script"/> and
    /// <see cref="FieldKind.Button"/>. C# evaluated via
...
public enum ExportFileMode
{
    ...
    /// <summary>Uses the successfully applied redacted copy when present, otherwise deliberately
    /// falls back to the original document.</summary>
    RedactedOrOriginal = 3
}
