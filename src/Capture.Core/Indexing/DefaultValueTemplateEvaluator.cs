...
/// <summary>Evaluates a field's <c>DefaultValueTemplate</c> — plain text passes through
...
public static class DefaultValueTemplateEvaluator
{
    ...
    public static IReadOnlyList<string> ReferencedFields(string? template)
    {
        if (string.IsNullOrEmpty(template))
            return [];
        ...
        for (var index = 0; index < template.Length; index++)
        {
            ...
            if (!string.IsNullOrEmpty(name) && !IsKnownMacro(name))
                references.Add(name);
            index = close;
        }
    ...
    private static string ResolveToken(string token, DefaultValueContext context)
    {
        ...
        if (string.Equals(name, "Page#", StringComparison.OrdinalIgnoreCase))
            return FormatCounter(context.PageNumber, param);
        if (string.Equals(name, "PageCount", StringComparison.OrdinalIgnoreCase))
            return FormatCounter(context.PageCount, param);
        ...
        // Anything else is a field-name reference. ProfileApplicator resolves dependencies first and
        // rejects cycles, so fields may safely refer to another populated/computed field.
        return context.Fields.TryGetValue(name, out var value) ? value : string.Empty;
    }

    private static bool IsKnownMacro(string name) =>
        ...
        name.Equals("PageCount", StringComparison.OrdinalIgnoreCase) ||
        name.Equals("Date", StringComparison.OrdinalIgnoreCase) ||
        name.Equals("Time", StringComparison.OrdinalIgnoreCase) ||
        name.Equals("ProfileName", StringComparison.OrdinalIgnoreCase);
