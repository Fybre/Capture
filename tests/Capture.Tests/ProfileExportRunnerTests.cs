...
public class ProfileExportRunnerTests
{
    ...
    [Fact]
    public async Task Passes_batch_field_definitions_to_every_export_writer()
    {
        var writer = new FakeExportWriter();
        var runner = new ProfileExportRunner([writer]);
        var profile = new DocumentTypeDefinition
        ...
        var batchField = new IndexField { Name = "Student number" };

        await runner.RunAsync(profile, MakeDocument(), [], [batchField]);

        Assert.Same(batchField, Assert.Single(writer.SeenBatchFields));
    }
