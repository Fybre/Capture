...
public class PageManagementServiceTests
{
    ...
    [Fact]
    public async Task SplitDocumentAsync_transfers_page_data_and_preserves_source_provenance()
    {
        var env = await TestEnv.CreateAsync();
        var document = await env.CreateDocumentAsync(pageCount: 5, isPdf: true);
        document.ContentHash = "source-hash";
        ...
        [
            new IndexValue
            {
                ...
                FieldName = "First half",
                Value = "first",
                PageNumber = 2,
                Bounds = new ZoneRect { PageNumber = 2, Width = 0.1f, Height = 0.1f }
            },
            new IndexValue
            {
                FieldId = Guid.NewGuid(),
                FieldName = "Second half",
                Value = "second",
                PageNumber = 4,
                Bounds = new ZoneRect { PageNumber = 4, Width = 0.1f, Height = 0.1f }
            },
            new IndexValue
            {
                FieldId = Guid.NewGuid(), FieldName = "Document value", Value = "original only", PageNumber = 1
            }
        ]);
        await env.RedactionCandidates.SaveAsync(document.Id,
        [
            new RedactionCandidate { Label = "first", PageNumber = 2, Width = 0.1f, Height = 0.1f },
            new RedactionCandidate { Label = "second", PageNumber = 4, Width = 0.1f, Height = 0.1f }
        ]);

        var (first, second) = await env.Service.SplitDocumentAsync(document.Id, splitBeforePageNumber: 3);

        Assert.Equal(document.ContentHash, second.ContentHash);
        Assert.Equal(document.SourceImportId, second.SourceImportId);

        var firstValues = await env.IndexValues.GetAsync(first.Id);
        Assert.Contains(firstValues, value => value.FieldName == "First half" && value.PageNumber == 2);
        Assert.Contains(firstValues, value => value.FieldName == "Document value");
        Assert.DoesNotContain(firstValues, value => value.FieldName == "Second half");

        var secondValues = await env.IndexValues.GetAsync(second.Id);
        Assert.Equal(3, secondValues.Count);
        var inheritedHeaderValue = Assert.Single(secondValues, value => value.FieldName == "First half");
        Assert.Equal("first", inheritedHeaderValue.Value);
        Assert.Equal(1, inheritedHeaderValue.PageNumber);
        Assert.Null(inheritedHeaderValue.Bounds); // source header page is not in this half
        var inheritedDocumentValue = Assert.Single(secondValues, value => value.FieldName == "Document value");
        Assert.Equal("original only", inheritedDocumentValue.Value);
        Assert.Null(inheritedDocumentValue.Bounds);
        var secondValue = Assert.Single(secondValues, value => value.FieldName == "Second half");
        Assert.Equal("Second half", secondValue.FieldName);
        Assert.Equal(2, secondValue.PageNumber);
        Assert.Equal(2, secondValue.Bounds!.PageNumber);

        Assert.Equal("first", Assert.Single(await env.RedactionCandidates.GetAsync(first.Id)).Label);
        var secondCandidate = Assert.Single(await env.RedactionCandidates.GetAsync(second.Id));
        Assert.Equal("second", secondCandidate.Label);
        Assert.Equal(2, secondCandidate.PageNumber);
        Assert.Equal(RedactionStatus.PendingReview, (await env.Store.GetAsync(second.Id))!.RedactionStatus);
    }
    ...
    [Fact]
    public async Task MergeDocumentsAsync_clears_source_provenance_because_the_content_has_changed()
    {
        var env = await TestEnv.CreateAsync();
        var first = await env.CreateDocumentAsync(pageCount: 1, isPdf: true);
        var second = await env.CreateDocumentAsync(pageCount: 1, isPdf: true);
        first.ContentHash = "first-hash";
        first.SourceImportId = Guid.NewGuid();
        second.ContentHash = "second-hash";
        second.SourceImportId = Guid.NewGuid();
        await env.Store.SaveAsync(first, await env.Store.GetPagesAsync(first.Id));
        await env.Store.SaveAsync(second, await env.Store.GetPagesAsync(second.Id));

        var merged = await env.Service.MergeDocumentsAsync([first.Id, second.Id]);

        Assert.Null(merged.ContentHash);
        Assert.Null(merged.SourceImportId);
        var persisted = await env.Store.GetAsync(merged.Id);
