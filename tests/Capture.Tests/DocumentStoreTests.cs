...
public class DocumentStoreTests
{
    ...
    [Fact]
    public async Task Soft_deleting_the_last_active_document_closes_but_preserves_its_batch()
    {
        var root = Path.Combine(Path.GetTempPath(), "capture-softdel-batch-" + Guid.NewGuid().ToString("N"));
        var paths = new AppPaths(root);
        paths.EnsureCreated();
        var store = new SqliteDocumentStore(paths);
        await store.InitializeAsync();
        var profileId = Guid.NewGuid();
        var batch = await store.CreateScopedBatchAsync(profileId, "manual");
        var first = new CaptureDocument { OriginalFileName = "one.pdf", StoredPath = "one.pdf", Source = DocumentSource.Import, BatchId = batch.Id };
        var second = new CaptureDocument { OriginalFileName = "two.pdf", StoredPath = "two.pdf", Source = DocumentSource.Import, BatchId = batch.Id };
        await store.SaveAsync(first, []);
        await store.SaveAsync(second, []);

        await store.SoftDeleteAsync(first.Id);
        Assert.Equal(batch.Id, (await store.GetOpenBatchAsync(profileId, "manual"))?.Id);

        await store.SoftDeleteAsync(second.Id);
        Assert.Null(await store.GetOpenBatchAsync(profileId, "manual"));
        Assert.Equal(2, (await store.GetTrashedAsync()).Count);

        await store.RestoreAsync(second.Id);
        Assert.Null(await store.GetOpenBatchAsync(profileId, "manual"));
        Assert.Equal(batch.Id, Assert.Single(await store.GetAllAsync()).BatchId);
    }
    ...
    public async Task ContentHash_and_source_import_id_roundtrip_through_save_and_update()
    {
        ...
        Assert.Equal(sourceImportId, saved.SourceImportId);
        ...
        Assert.Equal(sourceImportId, updated.SourceImportId);
    }
