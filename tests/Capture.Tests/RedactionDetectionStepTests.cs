...
public class RedactionDetectionStepTests
{
    ...
    [Fact]
    public async Task Failed_redaction_application_clears_and_removes_the_previous_artifact()
    {
        var root = Path.Combine(Path.GetTempPath(), "capture-redaction-apply-" + Guid.NewGuid().ToString("N"));
        ...
        var document = new CaptureDocument
        {
            ...
            RedactedPath = paths.DocumentRedactedPath(Guid.NewGuid())
        };
        Directory.CreateDirectory(Path.GetDirectoryName(document.RedactedPath)!);
        await File.WriteAllTextAsync(document.RedactedPath, "old redaction");
        ...
        [
            new DocumentPage
            {
                ...
                PageNumber = 1,
                SourcePageNumber = 1,
                ImagePath = Path.Combine(root, "page.png")
            }
        ]);
        var oldPath = document.RedactedPath;
        var applier = new RedactionApplier(new ThrowingRedactedDocumentWriter(), store, paths);

        await applier.ApplyAsync(document, await store.GetPagesAsync(document.Id), []);

        var persisted = await store.GetAsync(document.Id);
        Assert.Equal(RedactionStatus.Failed, persisted!.RedactionStatus);
        Assert.Null(persisted.RedactedPath);
    ...
    private sealed class ThrowingRedactedDocumentWriter : IRedactedDocumentWriter
    {
        public Task<string> WriteAsync(
            CaptureDocument document,
            IReadOnlyList<DocumentPage> pages,
            ...
            string outputPath,
            CancellationToken cancellationToken = default) =>
            throw new IOException("Simulated redaction write failure.");
    }
}
