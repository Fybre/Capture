...
public class ProfileApplicatorScriptTests
{
    ...
    [Fact]
    public async Task Post_process_script_can_explicitly_replace_the_fields_confidence()
    {
        var profile = new DocumentTypeDefinition
        {
            Fields =
            [
                new IndexField
                {
                    Name = "Category",
                    Kind = FieldKind.Text,
                    PostProcessScript = "\"General\""
                }
            ]
        };
        var runner = new FakeScriptRunner
        {
            OnFieldExpression = (_, _, _) => ScriptRunResult.Ok("General", TimeSpan.Zero, confidence: 100)
        };

        var value = Assert.Single(await new ProfileApplicator(scripts: runner).ApplyAsync(profile, []));

        Assert.Equal("General", value.Value);
    ...
    [Fact]
    public async Task Post_process_script_rewrites_a_captured_batch_separator_value()
    {
        var fieldId = Guid.NewGuid();
        ...
        var profile = new DocumentTypeDefinition
        {
            ...
            [
                ...
                {
                    Id = fieldId,
                    Name = "Separator Barcode",
                    Kind = FieldKind.BatchSeparatorValue,
                    BoundaryRuleId = ruleId,
                    PostProcessScript = "Value.Replace(\"Student|\", \"\")"
                }
            ]
        };
        string? valueSeenByScript = null;
        var runner = new FakeScriptRunner
        {
            OnFieldExpression = (_, _, context) =>
            {
                valueSeenByScript = context.Values.Single(value => value.FieldId == fieldId).Value;
                return ScriptRunResult.Ok("X00007", TimeSpan.Zero);
            }
        };
        var capturedValues = new[]
        {
            new IndexValue
            {
                FieldId = fieldId,
                FieldName = "Separator Barcode",
                Value = "Student|X00007",
                Confidence = 95
            }
        };

        var values = await new ProfileApplicator(scripts: runner).ApplyAsync(
            profile, [], existingValues: capturedValues);

        var value = Assert.Single(values);
        Assert.Equal("Student|X00007", valueSeenByScript);
        Assert.Equal("X00007", value.Value);
