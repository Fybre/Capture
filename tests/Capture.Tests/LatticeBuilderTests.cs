...
public class LatticeBuilderTests
{
    ...
    {
        var sample = Environment.GetEnvironmentVariable("CAPTURE_TEST_MIXED_PDF");
        if (string.IsNullOrWhiteSpace(sample) || !File.Exists(sample) || TesseractCliOcrEngine.ResolveExecutable() is null)
            return;
