...
public class RoslynFieldScriptRunnerTests
{
    ...
    [Fact]
    public async Task Scoped_context_is_available_while_fields_remains_a_compatible_alias()
    {
        var runner = new RoslynFieldScriptRunner(new AlwaysAllowed(), new HttpClient());
        var context = Context(new IndexValue { FieldId = Guid.NewGuid(), FieldName = "Type", Value = "" });
        context = new ScriptExecutionContext
        {
            ProfileName = context.ProfileName, DocumentNumber = 3, BatchNumber = 7, Timestamp = context.Timestamp,
            Values = context.Values, Document = context.Document, Scope = ScriptScopeKind.Document, DocumentType = "Invoice",
            TriggerMatches = [new ScriptTriggerMatchInfo(Guid.NewGuid(), "INV-42", 0.9)]
        };
        var script = new FieldScript { Source = "Fields[\"Type\"].Value = $\"{Context.Document.Type}:{Context.Batch == null}:{Context.Trigger.Matches[0].CapturedValue}\";" };

        var result = await runner.RunProfileScriptAsync(script, context);

        Assert.True(result.Success, result.ErrorMessage);
        Assert.Equal("Invoice:True:INV-42", context.Values.Single().Value);
    }
    ...
    [Fact]
    public async Task Field_expression_can_explicitly_set_its_own_confidence()
    {
        var fieldId = Guid.NewGuid();
        var runner = new RoslynFieldScriptRunner(new AlwaysAllowed(), new HttpClient());
        var context = Context(new IndexValue { FieldId = fieldId, FieldName = "Category", Value = "" });

        var result = await runner.RunFieldExpressionAsync(
            fieldId,
            "SetConfidence(100); \"General\"",
            context);

        Assert.True(result.Success, result.ErrorMessage);
        Assert.Equal("General", result.Value);
        Assert.Equal(100, result.Confidence);
        Assert.Equal(0, context.Values.Single().Confidence);
    }

    [Fact]
    public async Task Field_expression_rejects_an_invalid_confidence()
    {
        var runner = new RoslynFieldScriptRunner(new AlwaysAllowed(), new HttpClient());

        var result = await runner.RunFieldExpressionAsync(
            Guid.NewGuid(),
            "SetConfidence(101); \"value\"",
            Context());
    ...
    [Fact]
    public async Task Field_expression_accepts_a_trailing_semicolon_and_returns_its_value()
    {
        var fieldId = Guid.NewGuid();
        var runner = new RoslynFieldScriptRunner(new AlwaysAllowed(), new HttpClient());
        var context = Context(new IndexValue
        {
            FieldId = fieldId,
            FieldName = "Student No",
            Value = "Student|X00007"
        });

        var result = await runner.RunFieldExpressionAsync(
            fieldId,
            "Value.Replace(\"Student|\", \"\");",
            context);

        Assert.True(result.Success, result.ErrorMessage);
        Assert.Equal("X00007", result.Value);
    }
